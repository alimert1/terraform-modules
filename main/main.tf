module "vpc" {
    ...
    public-subnet-map = [{ name = "${var.env}-Public-1a", az = "${var.region}a", cidr = "10.0.0.0/24", rt_name =  "${var.env}-Public"},
                       { name = "${var.env}-Public-1b", az = "${var.region}b", cidr = "10.0.1.0/24" }]

    private-subnet-map = [{ name = "${var.env}-Private-1a", az = "${var.region}a", cidr = "10.0.10.0/24", rt_name =  "${var.env}-Private" },
                       { name = "${var.env}-Private-1b", az = "${var.region}b", cidr = "10.0.11.0/24" }]

    data-subnet-map = [{ name = "${var.env}-Data-1a", az = "${var.region}a" , cidr = "10.0.20.0/24", rt_name =  "${var.env}-Data"},
                       { name = "${var.env}-Data-1b" , az = "${var.region}b", cidr = "10.0.21.0/24"}]

    ecs-subnet-map = [{ name = "${var.env}-ecs-1a", az = "${var.region}a" , cidr = "10.0.30.0/24", rt_name =  "${var.env}-ecs"},
                       { name = "${var.env}-ecs-1b" , az = "${var.region}b", cidr = "10.0.31.0/24"}]
}
module "ecs" {  
    source = "../modules/ecs"
